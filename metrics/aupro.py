    ...
    def compute_pro(self, cca: Tensor, target: Tensor, preds: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if self.num_thresholds is not None:
            ...
            if torch.all((0 <= preds) * (preds <= 1)):
                thresholds = thresholds_between_0_and_1(self.num_thresholds, self.cuda_device)
            else:
                thresholds = thresholds_between_min_and_max(preds, self.num_thresholds, self.cuda_device)
